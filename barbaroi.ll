...
(define! (root-environment) (program? O)
        (or (primitive? O) (closure? O) (continuation? O)))

(define! (root-environment) reset (vov ((BODY copy-list) (ENV environment))
        (eval (cons prompt (cons () BODY)) ENV)))

(define! (root-environment) shift (vov ((LABEL copy) (BODY copy-list) (ENV environment))
        (signature/assert! LABEL symbol?)
        (eval (cons break (cons () (cons LABEL BODY))) ENV)))
...
(define! (root-environment) (dynamic-wind ENTER ACTION EXIT)
        (let restart ((THUNK (lambda () (reset (ACTION)))))
                ...
                (EXIT)
                (if (continuation? R)
                        (do     (define-here! ARGUMENT (shift ESCAPE ESCAPE))
                                (restart (lambda () (R ARGUMENT))))
                        R)))

(-: done --- make *init-environment* immutable? :-)))
