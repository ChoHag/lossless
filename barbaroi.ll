...
(define! (root-environment) (environment? O)
        (clop-predicate? *imp/environment?* O))

(define! (root-environment) (define-pending-return-record! WHERE)
        (signature/assert! WHERE environment?)
        (define-here! <pending-return> (new-record value continue))
        (define-here! (pending-return? O) (record/instance-of? <pending-return> O))
        (define-here! (new-pending-return VALUE CONTINUE)
                (-: signature/assert! CONTINUE continuation? TODO: write `continuation?' :-)
                (let ((R (record/new <pending-return>)))
                        (record/set! R 'value VALUE)
                        (record/set! R 'continue CONTINUE)
                        R))
        (define-here! (pending-return/value O)
                (signature/assert! O pending-return?)
                (record/get R 'value))
        (define-here! (pending-return/continue O)
                (signature/assert! O pending-return?)
                (record/get R 'continue))

        (define! WHERE <pending-return> <pending-return>)
        (define! WHERE pending-return? pending-return?)
        (define! WHERE new-pending-return new-pending-return)
        (define! WHERE pending-return/value pending-return/value)
        (define! WHERE pending-return/continue pending-return/continue)
        (define! WHERE pr/value pending-return/value)
        (define! WHERE pr/continue pending-return/continue))

(define-pending-return-record! (root-environment))

(define! (root-environment) (dynamic-wind ENTER ACTION EXIT)
        (define-pending-return-record! (current-environment))
        (define-here! (yield VALUE)
                (shift CONT (new-pending-return VALUE CONT)))
        (let restart ((THUNK (lambda () (reset (ACTION)))))
                (ENTER)
                (let ((R (THUNK)))
                        (EXIT)
                        (if (pending-return? R)
                                (let ((RE-THUNK (yield! (pr/value R))))
                                        (restart (lambda () ((pr/continue R) RE-THUNK))))
                                R))))
